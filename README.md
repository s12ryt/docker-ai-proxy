  ...
  "clients": [
    {
      ...
      "concurrent_limit": 0,
      "allowed_models": [],
      "note": "daily/rpm/concurrent limit=0 表示不限制；空 allowed_models 表示可使用全部模型",
      "created_at": ""
    }
  ],
...
| `GET /api/clients` | 取得具名稱、啟用狀態、每日/每分鐘/同時請求限制與允許模型白名單的 Client 清單 |
| `PUT /api/clients` | 建立、編輯或刪除 Clients（需 JSON Content-Type；停用 Token 會被拒絕，`daily_limit`、`rpm_limit`、`concurrent_limit` 與 `allowed_models` 會在 `/v1/*` 強制執行） |
...
模型代理端點（`/v1/*`、`/v1beta/*`）在 proxy 內部產生的錯誤通常會回 JSON：
...
授權中介層與管理端點（`/api/*`）部分錯誤會使用純文字回應。常見狀態碼如下：

| HTTP 狀態 | 可能訊息 | 常見原因 / 處理方式 |
| --- | --- | --- |
| `400 Bad Request` | `invalid JSON: ...`、`invalid json: ...`、`read body: ...`、`missing 'model'`、`missing model`、`inspect model: ...` | JSON 格式錯誤、body 讀取失敗、缺少 `model`，或 Client policy 檢查請求模型時失敗；檢查 `Content-Type`、JSON body 與模型欄位。 |
| `400 Bad Request` | `client ... token cannot contain whitespace`、`client ... is duplicated`、`access token ... is duplicated`、provider/client validation 訊息 | 儀表板或管理 API 儲存設定時驗證失敗；移除重複項、空白字元或補齊必要欄位。 |
| `401 Unauthorized` | `access token is not configured` | 沒有任何可用 Client 或 legacy `ACCESS_TOKENS`，系統 fail-closed；到儀表板「Client 管理」建立啟用中的 Client，或設定 legacy `ACCESS_TOKENS`。 |
| `401 Unauthorized` | `unauthorized` | Bearer Token 缺失、錯誤或對應 Client 已停用；確認 `Authorization: Bearer <token>` 與 Client 狀態。 |
| `401 Unauthorized` | `invalid username or password`、`admin token required` | 管理登入失敗，或 legacy 管理 API 未提供 `ADMIN_TOKEN`。 |
| `403 Forbidden` | `model not allowed for client` | Client 的 `allowed_models` 不包含請求模型；調整 Client 模型白名單或改用允許的模型。 |
| `403 Forbidden` | `admin required` | 目前登入帳號不是 admin，無權呼叫管理端點。 |
| `403 Forbidden` | `origin rejected`、`referer rejected`、`origin or referer required` | Cookie 登入的 mutating `/api/*` 請求未通過同源 CSRF 檢查；從同源 Dashboard 呼叫，或確認 `Origin` / `Referer`。 |
| `405 Method Not Allowed` | `method not allowed` | 使用了不支援的 HTTP method；依端點表改用正確 method。 |
| `415 Unsupported Media Type` | `application/json content-type required` | mutating `/api/*` 請求缺少 `Content-Type: application/json`。 |
| `429 Too Many Requests` | `client daily limit exceeded`、`client rpm limit exceeded`、`client concurrent limit exceeded` | Client 已達 UTC 當日 `daily_limit`、最近一分鐘 `rpm_limit`，或同時進行中的請求數已達 `concurrent_limit`；提高限制、稍後重試，或換用其他 Client。 |
| `500 Internal Server Error` | `marshal payload: ...`、設定檔 `read/write/parse/marshal` 錯誤、其他內部錯誤 | 伺服器內部處理或設定檔讀寫失敗；檢查 server log、`CONFIG_PATH` 權限與設定檔內容。 |
| `501 Not Implemented` | `... is only supported for OpenAI-compatible providers` | 對非 OpenAI-compatible provider 呼叫 `/v1/responses`、embeddings、completions、images 或 audio 透傳端點；改用 OpenAI-compatible provider 或 chat 轉譯端點。 |
| `502 Bad Gateway` | `upstream error: ...`、`read upstream: ...`、`translate response: ...` | 上游 provider 連線、狀態碼或回應轉譯失敗；檢查 provider API key、base URL、模型名稱與上游服務狀態。 |
| `503 Service Unavailable` | `usage store unavailable` | Client 設有 `daily_limit` 或 `rpm_limit`，但使用量資料庫不可用；檢查 DB 連線與 `ENABLE_METRICS` / DB 設定。 |
...
- **發布到公網前務必透過儀表板「Client 管理」建立至少一個啟用中的 Client，或保留 legacy `ACCESS_TOKENS`**；若沒有任何可用憑證，`/v1/*` 會回 `401`，不會開放匿名模型代理呼叫；Client 的 `daily_limit` 以 UTC 當日呼叫次數強制限制，`rpm_limit` 以最近一分鐘呼叫數限制，`concurrent_limit` 會限制同時進行中的請求/SSE 串流，`allowed_models` 會限制可用模型並過濾 `/v1/models` 清單
