...
| `429 Too Many Requests` | `client daily limit exceeded` | Client 已達 `daily_limit`（UTC 當日呼叫數）；提高限制、等到下一個 UTC 日，或換用其他 Client。 |
...
| `503 Service Unavailable` | `usage store unavailable` | Client 設有 `daily_limit`，但使用量資料庫不可用；檢查 DB 連線與 `ENABLE_METRICS` / DB 設定。 |
